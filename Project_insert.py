...
lecturers = [(i, fake.first_name(), fake.last_name()) for i in range(1001, 1041)]
...
# lecturere only does max 5 courses
course_count = {lecturer[0]: 0 for lecturer in lecturers}
lcourses = [[] for _ in range(len(lecturers))]
for i, course in enumerate(courses):
    random_lec = random.randint(0, len(lecturers) - 1)
    lid = lecturers[random_lec][0]
    if course_count[lid] < 5:
        lcourses[i].append((lid, i + 1))
        course_count[lid] += 1
    else:
...
# lecturer teaches at least 1 course
for i, courses_taught in enumerate(lcourses):
    if len(courses_taught) == 0:
        random_course = random.randint(0, len(courses) - 1)
        lcourses[i].append((lecturers[i][0], random_course + 1))

# SQL insert queries
with open('project_data.sql', 'w') as file:
    for student in students:
        file.write(f"INSERT INTO User (userID, fname, lname, role) VALUES ({student[0]}, '{student[1]}', '{student[2]}', 'Student');\n")

    for lecturer in lecturers:
        file.write(f"INSERT INTO User (userID, fname, lname, role) VALUES ({lecturer[0]}, '{lecturer[1]}', '{lecturer[2]}', 'Lecturer');\n")
    ...
    for i, enrol in enumerate(Enrol):
        for sid in students_in_course:
            file.write(f"INSERT INTO Enrol (userID, courseID) VALUES ({sid}, {i + 1});\n")

    for coursesTaught in lcourses:
        for lid, cid in courses_taught:
            file.write(f"INSERT INTO Teaches (userID, courseID) VALUES ({lid}, {cid});\n")